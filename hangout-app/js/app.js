...
function connectWS(){
  ...
  const url = proto+'//'+host+'/ws/'+encodeURIComponent(me.id);
  try{
...
async function seedIfEmpty(){
  ...
  let members=[me.id];
  for(const u of users){ if(u.id!==me.id && members.length<4) members.push(u.id); }
  const guestNames=['Mina','Ali','Reza'];
...
function renderDashboard(){
  ...
  el.innerHTML=`
    ...
    ${filtered.length===0? `<div class="empty">No hangouts in this filter. Create one for your 4!</div>` : `
    <div class="grid hangout-grid">
      ${filtered.map(h=>{
        ...
        return `<div class="card card-hover" style="cursor:pointer" onclick="openHangout('${h.id}')">
          ...
          <div style="font-weight:800;margin:8px 0">${h.title} ${h.roles && h.roles[me.id]==='host' ? '👑' : h.roles && h.roles[me.id]==='admin' ? '⭐' : ''}</div>
          <div class="subtle" style="font-size:13px;margin-bottom:8px">${h.description||'—'}</div>
          <div class="flex">${membersHtml}<span class="subtle" style="margin-left:6px">${h.members.length}/4 members</span></div>
  ...
  el.innerHTML=`
    ...
    <div class="card" style="margin-top:12px">
      <div class="flex justify-between" style="flex-wrap:wrap;gap:8px">
        <div><div style="font-size:22px;font-weight:900">${h.title} ${myRole==='host'?'👑':myRole==='admin'?'⭐':''}</div><div class="subtle">${h.description||''} · Your role: ${roleBadge(h, me.id)}</div></div>
      ...
      <div class="flex" style="margin-top:10px;flex-wrap:wrap;gap:6px">
        ${h.members.map(mid=>`<span class="badge" style="display:flex;align-items:center;gap:6px">${avatarHtml(mid,'xs')} ${memberName(mid)} ${roleBadge(h,mid)}</span>`).join('')}
...
function renderPlanTab(h, container){
  ...
  function pollList(title, items, type, icon){
    return `<div class="card" style="margin-bottom:12px">
      ...
      <div style="margin-top:10px" class="subtle">Each of 4 can vote for multiple options. Majority wins → finalize below.</div>
    </div>`;
  }
...
function renderBalancesTab(h, container){
  ...
  container.innerHTML=`
    ...
    <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px;margin-bottom:12px">
      ${h.members.map(mid=>{
        ...
        return `<div class="balance-card">
          <div class="flex">${avatarHtml(mid,'sm')}<b>${u?u.name:'?'}</b> ${roleBadge(h,mid)}</div>
    ...
    <div class="card" style="margin-bottom:12px">
      ...
      <div class="subtle" style="margin-top:8px">Algorithm: net = paid − owed, then greedy min-cash-flow (≤3 transfers for 4 people).</div>
    </div>
    ${h.payments.length? `
    <div class="card">
      ...
      <div class="grid" style="gap:8px;margin-top:8px">
        ${hangouts.map(h=>`
          <div style="padding:10px;background:var(--card2);border:1px solid var(--border);border-radius:10px">
            <div class="flex justify-between"><b>${h.title}</b><span class="badge">${h.members.length}/4</span></div>
...
function renderMemberPicker(){
  ...
  }).join('') + `<div class="subtle">${tempMembers.length}/4 selected</div>`;
}
function toggleMember(id){
  if(tempMembers.includes(id)) tempMembers=tempMembers.filter(x=>x!==id);
  else {
    if(tempMembers.length>=4) return alert('Max 4 members');
    tempMembers.push(id);
  }
  renderMemberPicker();
}
async function addCustomMember(){
  ...
  if(!name) return;
  if(tempMembers.length>=4) return alert('Max 4');
  if(USE_API){
    try{
      const g=await apiRegister(name, name.toLowerCase()+'@guest', 'guest');
      usersCache.push(g);
...
// Expose globals
window.switchAuth=switchAuth; window.doLogin=doLogin; window.doRegister=doRegister; window.logout=logout; window.openForgotModal=openForgotModal; window.doForgot=doForgot; window.hostResetPassword=hostResetPassword;
