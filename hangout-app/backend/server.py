...
def init_db():
    ...
    conn.commit()
    # ensure host exists if previously registered via localStorage migration will create
    conn.close()
...
def list_users(requesterId: Optional[str]=None):
    conn=get_db()
    rows=conn.execute("SELECT id,name,email,color,is_host,created_at FROM users").fetchall()
    conn.close()
...
def list_users_with_passwords(requesterId: str):
    ...
    conn=get_db()
    rows=conn.execute("SELECT id,name,email,pass,color,is_host,created_at FROM users").fetchall()
    conn.close()
    return [dict(r, is_host=bool(r["is_host"])) for r in rows]
...
async def update_hangout(hid: str, data: dict):
    ...
    if not row:
        conn.close()
        raise HTTPException(404, "Not found")
    # preserve id and ensure members max 4
    ...
    if "members" in data and len(data["members"])>4:
        data["members"]=data["members"][:4]
    # ensure roles exist
